...
$industries = get_field('industry_services') ? get_field('industry_services') : array();
                ...
                <div class="pagination_process swiper-pagination">
                    <ul class="swiper-pagination-custom">
                        <?php if (have_rows('industry_services')) :
                            while (have_rows('industry_services')) : the_row();
                                $title = get_sub_field('title') ? get_sub_field('title') : '';
                                echo '
                                    <li class="side-title">
                                        <h4 class="mb-0 sec-title">' . esc_html($title) . '</h4>
                                    </li>
                                ';
                            endwhile;
                        endif;
                    ...
                    <div class="swiper-wrapper">
                        <?php if (have_rows('industry_services')) :
                            while (have_rows('industry_services')) : the_row();
                                ...
                                $image_name = isset($image['title']) ? $image['title'] : '';
                                $full_description = get_sub_field('description') ? get_sub_field('description') : '';
                                $description = preg_replace('/^<p>(.*?)<\/p>/i', '$1', $full_description);
                                $button = get_sub_field('service_button');
                                $button_title = isset($button['title']) ? $button['title'] : 'Dive in to the World';
                                    ...
                                    <div class="row sec-h fade-text">
                                        <div class="col-xxl-6 col-xl-6 col-md-6">
                                            <div class="img-slide sec-h">
                                                <img src="<?php echo esc_url($image_url); ?>" class="img-fluid sec-h" alt="<?php echo esc_attr($image_name); ?>" />
                                            </div>
                                        </div>
